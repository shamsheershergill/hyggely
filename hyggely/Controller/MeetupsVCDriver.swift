...
class MeetupsVCDriver: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate, DriverController {
    ...
    override func viewDidLoad() {
        ...
        MeetupsHandlerDriver.Instance.delegate = self;
        MeetupsHandlerDriver.Instance.observeMessagesForDriver();
    ...
    func riderCanceledUber() {
        if !driverCanceledUber {
            MeetupsHandlerDriver.Instance.cancelUberForDriver();
    ...
    @objc func updateDriversLocation() {
        MeetupsHandlerDriver.Instance.updateDriverLocation(lat: userLocation!.latitude, long: userLocation!.longitude);
    }
    ...
    private func uberRequest(title: String, message: String, requestAlive: Bool) {
        ...
        if requestAlive {
            let accept = UIAlertAction(title: "Accept", style: .default, handler: { (alertAction: UIAlertAction) in
                ...
                self.timer = Timer.scheduledTimer(timeInterval: TimeInterval(10), target: self, selector: #selector(MeetupsVCDriver.updateDriversLocation), userInfo: nil, repeats: true);
                
                MeetupsHandlerDriver.Instance.uberAccepted(lat: Double(self.userLocation!.latitude), long: Double(self.userLocation!.longitude));
